...
# This also extracts current grade levels

# Run the MainScript first, but use the whole workbook

Workbook$pasteCredits = 0
Workbook$pasteGradeLevels = ""

for(i in 1:nrow(Workbook)){
  currentID = Workbook$`Local.ID.(optional)`[i]
  credits = sum(F2$EarnedCrHrs[F2$`[1]Student_Number` == currentID])
  Workbook$pasteCredits[i] = betterMax(c(credits, Workbook$Total.Credits.Earned[i]))
  if(Workbook$`Still.Enrolled?`[i] == "yes"){
    Workbook$pasteGradeLevels[i] = as.character(powerschoolraw$grade_level[match(Workbook$`Local.ID.(optional)`[i], table = powerschoolraw$student_number)])
  }
}
